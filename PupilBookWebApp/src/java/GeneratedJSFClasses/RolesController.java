...
import GeneratedSessionBeans.RolesFacade;
...
public class RolesController implements Serializable {
    ...
    @EJB
    private GeneratedSessionBeans.RolesFacade ejbFacade;
    ...
    protected void setEmbeddableKeys() {
        selected.getRolesPK().setUsersLogin(selected.getUsers().getLogin());
    }

    protected void initializeEmbeddableKey() {
        selected.setRolesPK(new GeneratedEntityClasses.RolesPK());
    }
    ...
    public static class RolesControllerConverter implements Converter {
        ...
        GeneratedEntityClasses.RolesPK getKey(String value) {
            GeneratedEntityClasses.RolesPK key;
            String values[] = value.split(SEPARATOR_ESCAPED);
            key = new GeneratedEntityClasses.RolesPK();
            key.setRole(values[0]);
            key.setUsersLogin(values[1]);
            return key;
        }

        String getStringKey(GeneratedEntityClasses.RolesPK value) {
            StringBuilder sb = new StringBuilder();
            sb.append(value.getRole());
            sb.append(SEPARATOR);
            sb.append(value.getUsersLogin());
            return sb.toString();
        }
